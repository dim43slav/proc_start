...
class ProcStart:
    ...
    def check_empty(self):
        ...
        wa_dataset_dp_wait_time = [row[3] for row in wa_dataset_dp][0]

        if wa_dataset_dp_wait_time <= 0:
                wa_dataset_dp_wait_time = self.c_max_wait_time_sec

        wa_dataset_dp_dataset = [row[0] for row in wa_dataset_dp][0]
        wa_dataset_dataset = wa_dataset_dp_dataset

        wa_dataset_dp_scenario = [row[1] for row in wa_dataset_dp][0]
        ...
        wa_dataset = query.query(f'''SELECT *
                                    FROM ddogadkin.zwt_dataset1
                                    WHERE
                                        dataset = \'{wa_dataset_dp_dataset}\' AND
                                        scenario = \'{wa_dataset_dp_scenario}\'
                                    LIMIT 1 FOR UPDATE''', 'S')
        #su-sybrc 8??

        wa_dataset_procstatus = [row[3] for row in wa_dataset][0]
        wa_dataset_timestamp = [row[4] for row in wa_dataset][0]
        ...
        if wa_dataset_procstatus == 'E':
            ...
            wa_dataset_ts_timestamp = [row[3] for row in wa_dataset_ts][0]
